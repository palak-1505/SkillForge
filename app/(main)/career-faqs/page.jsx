...
export default function CareerFaqs() {
  ...
  return (
    ...
    <div className="min-h-screen w-full bg-[#0A0A0A] text-white flex flex-col items-center px-6 pt-28 pb-16">
...
>
  BTech CS Career & Academic FAQs
      ...
      <div className="max-w-4xl w-full mt-10 space-y-8">
        {Object.keys(faqsByYear).map((year) => (
          <div key={year}>
            <h2 className="text-xl md:text-2xl font-semibold text-indigo-400 mb-4">
              {year}
            </h2>
            <div className="space-y-4">
              {faqsByYear[year].map((faq, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  <div
                    className={`transition-all duration-500 px-5 overflow-hidden ${
                      openIndex === index
